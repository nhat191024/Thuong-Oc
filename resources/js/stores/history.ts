...
export const useHistoryStore = defineStore('history', () => {
    const dishes = ref<orderDish[]>(isClient ? JSON.parse(localStorage.getItem('historyDishes') || '[]') : []);
    const totalPrice = ref<number>(isClient ? JSON.parse(localStorage.getItem('historyTotalPrice') || '0') : 0);
